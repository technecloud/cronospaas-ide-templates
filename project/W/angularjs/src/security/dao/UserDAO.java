...
public class UserDAO extends BasicDAO<String, User> {
   ...
   *
   * @param entitymanager Tabela do banco
   */
  ...
  /**
   * Busca um determinado usuário pelo nome de login e senha.
   *
   * @param login
   * @param password
   * @return
  ...
  public User findByLogin(String login, String password) {
    Query query = this.entityManager.createNamedQuery("userFindByLoginAndPassword");
    query.setParameter("login", login);
    query.setParameter("password", Hash.md5(password));
    return (User) query.getSingleResult();
  }
